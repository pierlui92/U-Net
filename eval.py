...
parser.add_argument('--pred_path',  type=str,   help='folder containing predictions semantic maps',   required=True)
parser.add_argument('--filenames_file', type=str,  help='path to filelist.txt', required=True)
parser.add_argument('--format_pred', type=str, choices=['id','trainId'], default='trainId',help='encoding of predictions, trainId or id')
parser.add_argument('--format_gt', type=str, choices=['id','trainId'], default='trainId',help='encoding of gt, trainId or id')
...
if args.format_pred == 'id':
    sem_pred = convert_to_train_id(sem_seg_placeholder)
else:
    sem_pred=sem_seg_placeholder

if args.format_gt == 'id':
    sem_gt = convert_to_train_id(sem_gt_placeholder)
else:
    sem_gt=sem_gt_placeholder
...
with tf.Session() as sess:
    ...
    for f in os.listdir(args.pred_path):
        if f.endswith(".png"):
            ...
            predictions[basename]=os.path.join(args.pred_path,f)
    ...
    for cl in range(confusion_matrix.shape[0]):
        ...
        print(trainId2name[cl], ": {:.4f}".format(IoU_cl))
    print("#######################")
